        ...
        {FindResult, _} ->
            case update_permissions(State, [Addr]) of
                {ok, NewState0} ->
                    ...
                    ?LOG_INFO("~s TURN channel ~.16B for peer ~s (lifetime: ~B seconds)",
                              [_Op, Channel, stun_logger:encode_addr(Peer), ?CHANNEL_LIFETIME div 1000]),
                    R = Resp#stun{class = response},
                    {next_state, active, send(NewState, R)};
                {error, Code} ->
                    Err = {_, Txt} = stun_codec:error(Code),
        ...
        {true, false} ->
            Perms =
                lists:foldl(fun(Addr, Acc) ->
                               ...
                               ?LOG_INFO("~s TURN permission for ~s (lifetime: ~B seconds)",
                                         [_Op, stun_logger:encode_addr(Addr), ?PERMISSION_LIFETIME div 1000]),
